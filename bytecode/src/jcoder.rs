...
/// assert_eq!((&mut cursor).read_u16().unwrap(), 0xBEEFu16);
/// assert_eq!((&mut cursor).read_u32().unwrap(), 0xCAFEBABEu32);
/// assert_eq!((&mut cursor).read_u64().unwrap(), 0x7080201020902412u64);
/// assert_eq!((&mut cursor).read_u128().unwrap(), 0x101112131415161718191A1B1C1D1E1Fu128)
/// ```
...
pub trait JEncoder: Write {
    ...
    fn write_mutf(&mut self, str: &str) -> Result<()> {
